#!/bin/bash
#
# Navencrypt automated-configuration script. This assumes Navencrypt is already installed.
#
...
storage=( "/dev/sdb" "/dev/sdb" "/dev/sdb" "/dev/sdc" "/dev/sdc" "/dev/sdc" "/dev/sdd" "/dev/sdd" "/dev/sdd" "/dev/sde" "/dev/sde" "/dev/sde" "/dev/sdf" "/dev/sdf" "/dev/sdf" "/dev/sdg" "/dev/sdg" "/dev/sdg" "/dev/sdh" "/dev/sdh" "/dev/sdh" "/dev/sdi" "/dev/sdi" "/dev/sdi" "/dev/sdj" "/dev/sdj" "/dev/sdj" "/dev/sdk" "/dev/sdk" "/dev/sdk"  )
mount=( "/data/01/.encrypted01" "/data/01/.encrypted01" "/data/01/.encrypted01" "/data/02/.encrypted02" "/data/02/.encrypted02" "/data/02/.encrypted02" "/data/03/.encrypted03" "/data/03/.encrypted03" "/data/03/.encrypted03" "/data/04/.encrypted04" "/data/04/.encrypted04" "/data/04/.encrypted04" "/data/05/.encrypted05" "/data/05/.encrypted05" "/data/05/.encrypted05" "/data/06/.encrypted06" "/data/06/.encrypted06" "/data/06/.encrypted06" "/data/07/.encrypted07" "/data/07/.encrypted07" "/data/07/.encrypted07" "/data/08/.encrypted08" "/data/08/.encrypted08" "/data/08/.encrypted08" "/data/09/.encrypted09" "/data/09/.encrypted09" "/data/09/.encrypted09" "/data/10/.encrypted10" "/data/10/.encrypted10" "/data/10/.encrypted10"  )

# Data to encrypt/protect
to_encrypt=( "/data/01/dfs" "/data/01/yarn" "/data/01/impala" "/data/02/dfs" "/data/02/yarn" "/data/02/impala" "/data/03/dfs" "/data/03/yarn" "/data/03/impala" "/data/04/dfs" "/data/04/yarn" "/data/04/impala" "/data/05/dfs" "/data/05/yarn" "/data/05/impala" "/data/06/dfs" "/data/06/yarn" "/data/06/impala" "/data/07/dfs" "/data/07/yarn" "/data/07/impala" "/data/08/dfs" "/data/08/yarn" "/data/08/impala" "/data/09/dfs" "/data/09/yarn" "/data/09/impala" "/data/10/dfs" "/data/10/yarn" "/data/10/impala"  )

# ACL categories to set
category=( "dfs" "yarn" "impala" "dfs" "yarn" "impala" "dfs" "yarn" "impala" "dfs" "yarn" "impala" "dfs" "yarn" "impala" "dfs" "yarn" "impala" "dfs" "yarn" "impala" "dfs" "yarn" "impala" "dfs" "yarn" "impala" "dfs" "yarn" "impala"  )
acl_binary=( "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*" "*"  )
...
# This variable controls whether or not Navencrypt performs SSL hostname verification
...
function registerClient {
    test -f /etc/navencrypt/keytrustee/clientname && printf "Client is already registered. Skipping registration.\n" && return
...
function prepareClient {
    ...
    while [[ $count -lt ${#storage[@]} ]]; do
        grep "${storage[$count]}\s" /etc/navencrypt/ztab &>/dev/null || grep "${mount[$count]}\s" /etc/navencrypt/ztab &>/dev/null
        if [[ $? -ne 0 ]]; then
...
function encryptData {
    ...
    while [[ $count -lt ${#to_encrypt[@]} ]]; do
        if [[ -L ${to_encrypt[$count]} ]]; then
            ...
            encrypt_command="navencrypt-move encrypt @${category[$count]} ${to_encrypt[$count]} ${mount[$count]}"
...
function addRules {
    ...
    while [[ $count -lt ${#acl_binary[@]} ]]; do
        cat $password_file | navencrypt acl --list | grep -r "${category[$count]}.*${acl_binary[$count]}" &>/dev/null
        if [[ $? -ne 0 ]]; then
            printf "Creating ACL rule for target [${acl_binary[$count]}], under the [@${category[$count]}] category\n"
            acl_command="navencrypt acl --add -r \"ALLOW @${category[$count]} * ${acl_binary[$count]}\""
...
function main {
    ...
    encryptData
    addRules
    printf "\nCompleted! Your Navencrypt password can be found at: $password_file\n"
}
