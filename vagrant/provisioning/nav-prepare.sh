...
for i in $(seq 1 $disks); do 
  mkdir -p /data/$i/{dfs,yarn,impala}
  mkdir /data/$i/.encrypted$i
done
...
echo "Setting variables for autoconfigure script"
driveletters=( "b" "c" "d" "e" "f" "g" "h" "i" "j" "k" )
storage=""
mount=""
to_encrypt=""
category=""
acl_binary=""
for i in $(seq 1 $disks); do
  for d in dfs yarn impala; do
    echo "Locations for $i $d"
    storage+="\"/dev/sd${driveletters[(( i - 1 ))]}\" "
    mount+="\"/data/${i}/.encrypted${i}\" "
    to_encrypt+="\"/data/${i}/${d}\" "
    category+="\"${d}\" "
    acl_binary+="\"*\" "
  done
...
echo "Writing autoconfigure script" >&2
cat > /root/navencrypt-autoconfigure.sh <<EOF
#!/bin/bash
#
